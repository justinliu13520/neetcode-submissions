...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        def dfs(l,r):
            # really going through preorder traversal to make the tree. We only use the inorder
            # to know when to break with the l > r
            if l > r:
                return None

            root = TreeNode(preorder[self.preorder_index]) # make the root with the current first in preorder once we increment
